"""JoinQuant play script for current ETF pool core guard.

复制到聚宽策略编辑器运行。本脚本是给用户试玩和公开平台复验用的
`daily_core_guard` 当前 ETF 池版本，不是实盘交易建议。

本地筛选参考：

- 数据：TuShare 日线，ETF 池 2024-01-01 至 2026-06-25。
- 候选：daily_core_guard，按 PASS 候选中最大回撤优先选择。
- 本地 sanity：年化约 10.44%，最大回撤约 9.61%，年化费用约 0.46%。

策略设计：

- ETF 池只使用用户当前关心的 ETF，不加入债券或类存款资产。
- 每日开盘评估，但常规调仓至少间隔 5 个交易日。
- 市场趋势和 ETF 池趋势广度转弱时清仓。
- 风险开启时只选 1 只 ETF，目标仓位 50%，其余保持现金。
- 单次开盘函数只运行一次，避免同日重复触发。
...
def initialize(context):
    ...
    g.strategy_id = "ETF-CUR-CORE-GUARD-PLAY"
    g.etf_pool = [
        "510300.XSHG",  # 沪深300 ETF
        "159659.XSHE",  # 纳指 ETF
        "513010.XSHG",  # 港股科技 ETF
        "515120.XSHG",  # 创新药 ETF
        "518880.XSHG",  # 黄金 ETF
        "512400.XSHG",  # 有色 ETF
    ]
