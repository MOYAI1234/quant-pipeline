...
`config init` 默认生成被 `.gitignore` 忽略的 `config.local.json`，已有文件不会被覆盖；只有显式添加 `--force` 才会覆盖。真实 API key、token 和私有 provider 路径应只保存在本地配置或环境变量中。生产或 CI 场景可使用 `--strict-warnings` 将风险 warning 视为失败。
...
- CLI `config validate` 对内置配置和 JSON 配置文件的校验，包括真实历史 provider 禁用缓存时的频率风险 warning 和 `--strict-warnings` 风险门禁
