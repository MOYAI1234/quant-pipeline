...
- `backtest/BacktestRunner` 已支持最小 grid 历史样例回测，`RotationBacktestRunner` 已支持内置多 ETF 轮动样例回测，并复用 `Simulator` 输出收益、最大回撤、最大回撤区间、交易次数、胜率和可配置滑点等基础指标；当前还不是完整回测系统，不含交易日历、复杂组合和真实历史数据源。
...
- `BacktestRunner` 的 grid 买卖周期、日期区间过滤、历史日期/盘中时间顺序校验、最大回撤区间、胜率统计、滑点执行价、轮动样例回测、空历史保护、CSV 读取/错误处理和 CLI smoke
