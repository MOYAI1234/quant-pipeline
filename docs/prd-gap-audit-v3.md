...
| 回测功能 | 已启动 | 已新增最小 `BacktestRunner`、`RotationBacktestRunner` 和 `BacktestExecutionModel`，支持历史 bar、CSV、JSON 驱动、基础回测指标、滑点、成交量参与率限制、拒单归因和逐期组合快照导出；仍缺交易所官方日历、部分成交、复杂组合、多策略和真实历史数据源 |
...
| 测试体系 | 不足 | 现有 267 个测试覆盖 simulator、grid e2e、rotation、risk manager 边界、backtest runner、回测成交模型、回测日期区间、历史日期/盘中时间顺序、OHLC 合法性、成交量参与率、组合快照一致性、拒单审计和可选交易日历校验、回测最大回撤区间、回测手续费统计、权益曲线/组合快照/成交明细 CSV 导出、回测滑点、CLI smoke、状态持久化、报告健康状态、告警事件、配置校验、DataManager 缓存与数据契约等；adapter、report 仍有缺口 |
...
1. 补充真实历史数据适配层到回测输入格式的转换脚本。
2. 继续补充 `README.md` 和专题文档，明确当前是 mock/simulator 阶段，不是实盘系统。
