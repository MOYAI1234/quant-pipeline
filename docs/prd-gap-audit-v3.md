...
| 监控告警 | 部分完成 | 状态指标、报告和结构化告警事件已存在，支持本地 JSONL 输出；外部通知通道尚未实现 |
| CLI | 基础可用 | `start/status/report/health/backtest` 已有；仍缺 config validate 等关键命令 |
| API/Web | 未完成 | PRD 中规划了 API 和 Web 界面，当前仓库没有对应模块 |
| 测试体系 | 不足 | 现有 108 个测试覆盖 simulator、grid e2e、rotation、backtest runner、CLI smoke、状态持久化、报告健康状态、告警事件等；risk、adapter、report 仍有缺口 |
...
状态：已启动。当前 CLI 已新增 `health` 命令，可输出数据源健康状态的文本报告，也可通过 `--json` 输出结构化摘要；`--strict` 可在任一 adapter 不可用时返回非零退出码。日报/周报也已包含数据源健康状态块。`AlertManager` 已支持结构化告警事件、内存历史和可选 JSONL 文件输出，`SystemMonitor` 会记录亏损和持仓阈值告警。该能力仍是 M4 起步版，尚未接入飞书、邮件等外部通知通道。
