...
但从 PRD v3 的完整目标看，当前项目仍不是“完整 ETF 量化交易系统”。策略/因子研究闭环、公开平台策略交叉验证、真实实时数据、券商订单状态机、QMT、API/Web 和外部通知仍未交付，因此不能把当前版本解释为生产实盘版。

下一阶段重点从“补工程地基”转为“策略/因子研究与外部验证”：先建立候选策略记录和因子假设模板，再用项目内回测做工程 sanity check，并在聚宽、优矿、米筐等公开平台验证关键策略假设。真实 provider 继续服务于本地研究和输入复核，但不替代公开平台的策略验证。
...
| 策略/因子研究闭环 | 部分完成 | 已新增研究工作流、候选策略模板、候选索引和首个 ETF 动量质量轮动候选记录；仍缺公开平台复现脚本和验证结果归档 |
...
项目已经具备工程自验证、内部回测和真实历史 provider 接入入口，并开始用候选策略索引记录第一个 ETF 因子轮动假设；但“策略来源 -> 候选记录 -> 本地 sanity check -> 公开平台验证 -> 模拟盘观察”的闭环还没有完成公开平台验证和结果归档。若继续优先扩展自研回测或执行系统，容易把工程能力误当作策略资产。

风险：

- 新策略来源、假设、参数和验证结果若不持续登记，仍会不可追溯
- 内部回测结果被过度解释为策略有效
- 无法系统化吸收 GitHub、研报、论文、公开平台和 AI 生成的新策略/因子
- 后续模拟盘或实盘缺少明确晋级门槛

建议：

- 使用[策略研究工作流](strategy-research-workflow.md)和[策略候选索引](strategy-candidates/README.md)作为下一阶段主线
- 为每个候选策略填写[策略候选模板](strategy-candidate-template.md)
- 先围绕首个 [ETF 动量质量轮动 v1](strategy-candidates/etf-momentum-rotation-v1.md) 写公开平台复现脚本
- 将项目内回测定位为工程 sanity check，把公开平台验证作为策略有效性的主要证据
...
### P0：内部回测已启动，但仍缺公开平台策略验证

PRD 将“可回测策略系统”列为阶段二交付物。当前仓库已经有内部回测 runner，用于验证本系统的策略状态机、风控、执行模型、报告和导出产物；但内部回测不能单独证明策略可生产，仍需要公开回测平台做外部结果交叉验证。下一阶段不应继续把重点放在扩展私有回测引擎，而应先把候选策略放到公开平台复现。
...
状态：下一阶段重点。已新增策略研究工作流、候选策略模板、候选索引和首个 ETF 动量质量轮动候选记录，但尚未形成公开平台复现产物。
...
- 维护候选策略目录或索引
- 围绕第一个 ETF 因子轮动策略补公开平台复现脚本和验证记录
- 明确因子定义、标的池、调仓频率、费用假设、风险约束和失效条件
- 准备聚宽、优矿或米筐的复现脚本或操作记录
- 将公开平台结果、项目内 sanity check 和差异解释归档到同一候选记录

验收：

- 每个策略都有来源、假设、参数、成本和风险记录
- 至少一个 ETF 因子策略完成公开平台验证记录
- 项目内回测只作为工程 sanity check，不单独作为策略有效性结论
- 高密度网格或成本后失效的策略被明确标记为 rejected 或 watchlist
...
研究/模拟版已达到 [交付基线](release-readiness.md)。下一轮不碰实盘接口，也不优先扩展自研回测引擎，进入策略/因子研究与公开平台验证：

1. 为首个 [ETF 动量质量轮动 v1](strategy-candidates/etf-momentum-rotation-v1.md) 准备聚宽、优矿或米筐的复现脚本或操作记录，并按[公开回测平台交叉验证](public-backtest-validation.md)归档结果。
2. 使用项目内真实 provider 和 backtest 做工程 sanity check，验证数据契约、费用、滑点、成交量参与率、止损和风控是否会推翻策略。
3. 将公开平台结果、本地 sanity check 和差异解释回填到候选记录。
4. 继续从 GitHub、论文、研报、公开平台示例和 AI 生成中补充新的候选策略，但新增候选必须先登记来源、许可证、因子、成本和风险。
5. 只有公开平台差异可解释且保守成本下结论不反转，才规划模拟盘或小资金实盘预研。
