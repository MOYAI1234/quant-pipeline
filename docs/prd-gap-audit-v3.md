...
| 测试体系 | 不足 | 现有 112 个测试覆盖 simulator、grid e2e、rotation、backtest runner、CLI smoke、状态持久化、报告健康状态、告警事件等；risk、adapter、report 仍有缺口 |
...
状态：已启动。当前 CLI 已新增 `health` 命令，可输出数据源健康状态的文本报告，也可通过 `--json` 输出结构化摘要；`--strict` 可在任一 adapter 不可用时返回非零退出码。日报/周报也已包含数据源健康状态块。`AlertManager` 已支持结构化告警事件、内存历史和可选 JSONL 文件输出，`SystemMonitor` 会记录亏损和持仓阈值告警，报告会展示最近告警摘要。该能力仍是 M4 起步版，尚未接入飞书、邮件等外部通知通道。
