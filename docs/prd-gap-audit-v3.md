...
- `compileall` 通过
- 离线 `pytest` 通过，`400 passed, 2 skipped`
- CLI help 可用
...
| 测试体系 | 研究版可交付 | 现有 400 个离线测试和 2 个默认跳过的 AKShare / TuShare guarded live test，覆盖策略状态机、风控、模拟执行、回测成交模型、数据契约、provider 降级、状态恢复、监控报告、CLI、回测产物 schema 与确定性；生产阶段仍需真实订单状态机、长时间运行和故障注入测试 |
