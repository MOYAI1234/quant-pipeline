...
| 模拟执行器 | 基础可用 | 买卖、整手、均价、买卖双边费率、单笔最低佣金、估值和部分盈亏计算已实现；回测层已抽出 `BacktestExecutionModel` 统一处理滑点、成交量参与率限制和成交前拒单归因，但仍不支持部分成交和更复杂撮合 |
...
| 测试体系 | 不足 | 现有 316 个测试覆盖 simulator、买卖双边费率和最低佣金、grid e2e、rotation、risk manager 边界、backtest runner、回测成交模型、回测历史转换、真实历史 provider 探测、回测日期区间、历史日期/盘中时间顺序、OHLC 合法性、成交量参与率、组合快照一致性、拒单审计和可选交易日历校验、回测最大回撤区间、回测手续费统计、权益曲线/组合快照/成交明细 CSV 导出、回测滑点、CLI smoke、配置模板生成、状态持久化、报告健康状态、启动前诊断、告警事件、配置校验、DataManager 缓存、数据契约和外部历史 provider 配置等；adapter、report 仍有缺口 |
...
1. 用实际 `mx_data.history` 命令 provider 配置生成一份真实回测输入样例；仓库已提供 `history probe` 作为不落盘最小查询入口，并记录不可提交凭据的运行说明。
2. 使用本地真实 provider 配置执行 `history probe`，完成环境侧 guarded e2e。
