...
下一阶段重点从“补工程地基”转为“稳健 ETF 策略/因子研究与外部验证”：先维护策略矩阵、回测台账和准入门槛，再用项目内回测做工程 sanity check，并在聚宽、优矿、米筐等公开平台验证关键策略假设。真实 provider 继续服务于本地研究和输入复核，但不替代公开平台的策略验证。
...
| 策略/因子研究闭环 | 部分完成 | 已新增研究工作流、候选策略模板、候选索引和稳健 ETF 策略研究台账；当前三条策略已作为 baseline/watchlist 归档，仍缺低回撤新候选、公开平台复现脚本和验证结果归档 |
...
- 维护候选策略目录或索引
- 围绕稳健 ETF 主线维护策略矩阵、回测台账、公开平台验证记录和准入门槛
