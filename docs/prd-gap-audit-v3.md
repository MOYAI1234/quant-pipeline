...
| 回测功能 | 已启动 | 已新增最小 `BacktestRunner` 和 `RotationBacktestRunner`，可用历史 bar / 多 ETF 样例驱动 grid、rotation 策略并复用 `Simulator` 输出收益、最大回撤、最大回撤区间、交易次数、胜率、权益曲线/成交明细 CSV 导出和可配置滑点等基础指标，CLI 已支持回测日期区间过滤和历史日期/盘中时间顺序校验；仍缺完整交易日历、复杂组合、多策略和真实历史数据源 |
