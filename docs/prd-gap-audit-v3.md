...
| 测试体系 | 不足 | 现有 266 个测试覆盖 simulator、grid e2e、rotation、risk manager 边界、backtest runner、回测成交模型、回测日期区间、历史日期/盘中时间顺序、OHLC 合法性、成交量参与率、拒单审计和可选交易日历校验、回测最大回撤区间、回测手续费统计、权益曲线/组合快照/成交明细 CSV 导出、回测滑点、CLI smoke、状态持久化、报告健康状态、告警事件、配置校验、DataManager 缓存与数据契约等；adapter、report 仍有缺口 |
