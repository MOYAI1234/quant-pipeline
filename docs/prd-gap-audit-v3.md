...
| 策略/因子研究闭环 | 未完成 | 已新增研究工作流和候选策略模板；仍缺候选策略登记索引、首个 ETF 因子策略记录、公开平台复现脚本和验证结果归档 |
...
项目已经具备工程自验证、内部回测和真实历史 provider 接入入口，但还没有把“策略来源 -> 候选记录 -> 本地 sanity check -> 公开平台验证 -> 模拟盘观察”的研究闭环固化下来。若继续优先扩展自研回测或执行系统，容易把工程能力误当作策略资产。
...
- 使用[策略研究工作流](strategy-research-workflow.md)作为下一阶段主线
- 为每个候选策略填写[策略候选模板](strategy-candidate-template.md)
- 先完成首个 ETF 因子轮动策略记录，再写公开平台复现脚本
...
状态：下一阶段重点。已新增策略研究工作流和候选策略模板，但尚未形成候选策略索引、首个 ETF 因子策略记录和公开平台复现产物。
...
1. 建立候选策略目录或索引，并把[策略候选模板](strategy-candidate-template.md)作为新增策略的必填记录。
2. 选择一个 ETF 因子轮动方向作为首个候选策略，写清楚来源、因子、标的池、调仓频率、成本、风险和失效条件。
3. 准备聚宽、优矿或米筐的复现脚本或操作记录，并按[公开回测平台交叉验证](public-backtest-validation.md)归档结果。
