...
| 回测功能 | 已启动 | 已新增最小 `BacktestRunner`、`RotationBacktestRunner` 和 `BacktestExecutionModel`，支持历史 bar、CSV、JSON 驱动、基础回测指标、滑点、成交量参与率限制和拒单归因；仍缺交易所官方日历、部分成交、复杂组合、多策略和真实历史数据源 |
...
| 测试体系 | 不足 | 现有 263 个测试覆盖 simulator、grid e2e、rotation、risk manager 边界、backtest runner、回测成交模型、回测日期区间、历史日期/盘中时间顺序、OHLC 合法性、成交量参与率、拒单审计和可选交易日历校验、回测最大回撤区间、回测手续费统计、权益曲线/成交明细 CSV 导出、回测滑点、CLI smoke、状态持久化、报告健康状态、告警事件、配置校验、DataManager 缓存与数据契约等；adapter、report 仍有缺口 |
...
- 输出产物：两类 runner 均复用 `Simulator` 输出收益、最大回撤、最大回撤区间、交易次数、拒单次数与原因、胜率、总手续费及其初始资金占比，并支持权益曲线、成交明细、持仓明细和拒单明细 CSV 导出。
...
1. 增加组合级回测结果导出，覆盖现金、总市值、持仓明细、交易明细和拒单明细的一致性校验。
2. 补充真实历史数据适配层到回测输入格式的转换脚本。
3. 继续补充 `README.md` 和专题文档，明确当前是 mock/simulator 阶段，不是实盘系统。
