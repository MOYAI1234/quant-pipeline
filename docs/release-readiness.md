...
截至 2026-07-13，当前代码已达到 **研究/模拟验证版交付基线**。这表示仓库具备可重复的离线验收、策略与风控回归、状态恢复、历史 provider 接入演练和回测产物审计能力；不表示策略已经通过外部市场验证，也不表示系统可接入生产实盘。测试数量会随实现演进，最新结果以 CI 和 `scripts/verify_offline.py` 输出为准。
...
- 离线测试基线由 CI 和 `scripts/verify_offline.py` 持续验证。
...
| 文档收口 | README、architecture、testing、策略研究工作流和 PRD gap 状态同步 | 完成 |
...
公开回测平台用于验证策略结果和市场假设，内部回测用于验证本系统的工程链路和成本约束。两者不能互相替代；当两者结论冲突时，优先调查数据、复权、交易日历、信号时点、费用和撮合模型差异，而不是直接扩大自研回测范围。
