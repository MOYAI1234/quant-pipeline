...
- 下一轮优先寻找低回撤 ETF 主线方向：ETF 趋势防守、核心池轮动、目标仓位控制、绝对动量降仓。
- 只有本地 sanity check 接近最大回撤 10%-15% 区间，且费用、滑点、换手和成交约束可解释，才进入公开平台验证。

## 2026-06-22 路线校准

用户明确要求保持 ETF 研究助手定位，不默认通过债券、黄金或类存款资产降低回撤；“低频”也不等于只能月频，而是低交易密度、不做日内反复交易。后续筛选应优先在权益 ETF 池内寻找趋势防守、相对强弱、目标仓位和空仓风控。

本轮曾新增 `ETF-EQ-MOM-GUARD-006` 作为公开平台验证候选；后续修复本地成交状态后，该候选结果失效，已降为趋势防守观察项。下一轮继续在权益 ETF 池内寻找低回撤、低交易密度的新因子。
