...
当前离线基线：`pytest` 应通过 396 个测试，并跳过 2 个显式启用的 AKShare / TuShare live test。
...
- `history probe` 能对真实命令 provider 执行不落盘的最小查询，并拒绝无数据、乱序或越界历史；`--json` 成功时应输出本次历史缓存统计和 provider 状态，失败时也必须输出 `available=false`、错误码、来源、错误消息、缓存统计和 provider 状态，便于 CI/provider 接入脚本消费。
...
- `tests/test_tushare_history_provider_live.py` 默认跳过；只有显式设置 `RUN_TUSHARE_LIVE=1`、`TUSHARE_TOKEN` 且安装 TuShare 后，才通过 CLI `history probe` 访问真实网络。
...
- CLI `diagnose` 能汇总配置校验、配置风险 warning、数据源健康状态和状态文件可恢复性；`--strict` 必须把 `config.strict_warnings` 作为启动前门禁失败，并在 JSON/text 输出中明确列出阻断原因。
