...
当前离线基线：`pytest` 应通过 359 个测试，并跳过 1 个显式启用的 AKShare live test。
...
- `config validate` 必须在真实历史 provider 启用且历史缓存 TTL 为 0 时给出 warning，并支持 `--strict-warnings` 将 warning 作为 CI/生产门禁失败。
